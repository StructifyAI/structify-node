...
import { ClientOptions } from 'structifyai';
...
export { ClientOptions } from 'structifyai';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'structifyai_api',
    version: '1.145.0',
  },
  {
    capabilities: {
...
export function initMcpServer(params: {
  ...
  const client = new Structify(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client =
    params.client ||
    new Structify({
      environment: (readEnv('STRUCTIFY_ENVIRONMENT') || undefined) as any,
